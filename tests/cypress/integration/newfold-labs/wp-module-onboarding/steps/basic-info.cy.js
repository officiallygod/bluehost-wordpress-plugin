...
describe('Basic Info Page', function () {
    ...
    it('Enter a Title and then Check if it reflects elsewhere', () => {
        ...
        cy.get(':nth-child(1) > label > .nfd-input__field').then((titleBox) => {
            cy.wrap(titleBox).scrollIntoView();
            cy.wrap(titleBox).clear();
            cy.wait(1000);
            cy.wrap(titleBox).type(title);
    ...
    it('Enter a Desc and then Check if it reflects elsewhere', () => {
        ...
        cy.get(':nth-child(2) > label > .nfd-input__field').then((descBox) => {
            cy.wrap(descBox).scrollIntoView();
            cy.wrap(descBox).clear();
            cy.wait(1000);
            cy.wrap(descBox).type(desc);
    ...
    it('Check if Social Media Accordion Toggles', () => {
        cy.get(':nth-child(7) > .social-form__label > .social-form__label_name')
            .should('exist')
            .should('not.be.visible');
    ...
    it('Check if Social Media URL checks are done', () => {
        ...
        cy.get('#facebook').then((socialTest) => {
            cy.wrap(socialTest).clear();
            cy.get(
                '[style="background-image: var(--facebook-colored-icon);"]'
            ).should('not.exist');

            cy.wrap(socialTest).type(invalidURL);
            ...
            cy.wrap(socialTest).clear();
            cy.wrap(socialTest).type(validURL);
